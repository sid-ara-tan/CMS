...
class Student_home extends CI_controller {
    ...
    function __construct() {
        ...
        $this->prefs_calender = array(
            'template' => '{table_open}<table border="0" cellpadding="0" cellspacing="0" class="calender">{/table_open}
                            ...
                            <div class="day_num highlight2">{day}</div>
                            {/cal_cell_no_content_today}
    ...
    function index() {
        ...
        /////////notification
        $i = 0;
        $notification_query = $this->notification->get_noti_students(5, 0);
        if ($notification_query != FALSE) {
            foreach ($notification_query->result_array() as $row) {
                ...
                $notification_array[$i]['CourseNo'] = $row['CourseNo'];
    ...
    function load_exam_calender() {
        ...
        if ($data['query_exam'] != FALSE)
            echo $this->load->view('student_home_page_exam_calender', $data, TRUE);
        else
            echo "No Exam Today";
    }
    ...
    function edit_profile() {
        ...
        $this->form_validation->set_rules('std_id', 'ID', 'trim|required|min_length[7]|max_length[7]|xss_clean'); //is_unique[users.username]
        $this->form_validation->set_rules('std_name', 'Name', 'trim|required|min_length[3]|max_length[50]|xss_clean');
        $this->form_validation->set_rules('Fname', 'Father\'s Name', 'trim|max_length[50]|xss_clean');
        $this->form_validation->set_rules('user_email', 'EMail', 'trim|required|valid_email|xss_clean');
        $this->form_validation->set_rules('user_address', 'Address', 'trim|max_length[320]|xss_clean');
        $this->form_validation->set_rules('user_phone', 'Phone Number', 'trim|numeric|max_length[20]|xss_clean');
        $this->form_validation->set_rules('password1', 'New Password', 'trim|matches[password2]|xss_clean');
        $this->form_validation->set_rules('password2', 'Confirm Password', 'trim|xss_clean');
        $this->form_validation->set_rules('passwordc', 'Current Password', 'trim|required|xss_clean');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($update) {
                ...
                $this->session->set_flashdata('notification', "Wrong Current Password ");
                redirect('student_home/profile');
            }
        }
    ...
    function load_notification() {
        ...
        $i = 0;
        $notification_query = $this->notification->get_noti_students($config['per_page'], $offset);
        if ($notification_query != FALSE) {
            foreach ($notification_query->result_array() as $row) {
                $notification_array[$i]['material'] = $row['material'];
                if ($notification_array[$i]['material'] == 'marks' || $notification_array[$i]['material'] == 'exam')
        ...
        $this->load->view('student_notification', $data);
    }
